...
class ChangingBias(Gaussian_Process.GaussianProcess):
    def __init__(self, space_X, time_X, _Y, space_Xt, time_Xt, _Yt, space_kernel, time_kernel, kernel, noise_sd, theta_not,
        ...
        sigma_hat_inv = torch.linalg.inv(sigma + (noise_lag ** 2) * torch.eye(len(sigma)))
        bias_sigma = torch.kron(torch.eye(len(space_X)), torch.eye(len(time_X)))
        ...
        A += (sigma_hat_inv).T
        A += torch.eye(N_sensors * N_time) * (alpha ** 2) / (bias_variance ** 2)
        C[0] = Y.T @ sigma_hat_inv + current_C + (bias_mean / (bias_variance ** 2)) * (alpha ** 2) * torch.ones(
            N_sensors * N_time)
