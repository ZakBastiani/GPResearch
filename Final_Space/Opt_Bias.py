...
class OptBias(Gaussian_Process.GaussianProcess):
    def __init__(self, space_X, time_X, _Y, space_Xt, time_Xt, _Yt,
        ...
        class opt_bias(nn.Module):
            def __init__(self, X, Y, N_sensors, N_time, Xt, Yt):
            ...
            def calcboth(self, Xt, Yt):
                alpha = 1
                b = torch.zeros((N_time * N_sensors, 1))
                ...
                for counter in range(20):
                    ...
                    for i in range(len(Xt)):
                        k_star = self.kernel(Xt[i].unsqueeze(0), X)
        ...
        theta_model = opt_bias(X, Y, len(space_X), len(time_X), Xt, Yt)
        optimizer = torch.optim.Adam(theta_model.parameters(), lr=0.00001)
        smallest_loss = 1000
        for i in range(500):
            optimizer.zero_grad()
