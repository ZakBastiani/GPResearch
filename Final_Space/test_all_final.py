...
def bias_kernel(X, Y):
    kernel = (bias_variance ** 2) * torch.exp(-((X.repeat(len(Y), 1) - Y.repeat(len(X), 1).T) ** 2) / (2 * theta_time ** 2))
    return kernel
...
for i in range(0, N_trials):
    ...
    all_sensor_points = torch.cat((sensors.repeat(N_time, 1), sensor_time.repeat_interleave(N_sensors).repeat(1, 1).T), 1)
    ...
    all_true_sensor_points = torch.cat((true_sensors.repeat(N_true_time, 1), true_sensor_time.repeat_interleave(N_true_sensors).repeat(1, 1).T), 1)
    ...
    gp_error = gp.print_error(alpha, sensor_bias, gaussian.underlyingData, estimate, true_data, gt_estimate)
    ...
    calc_alpha_errors += calc_alpha.print_error(alpha, sensor_bias, gaussian.underlyingData, calc_alpha_estimate, true_data, calc_alpha_gt_estimate)
    ...
    calc_changing_bias_error += changing_bias_gp.print_error(alpha, sensor_bias, gaussian.underlyingData, changing_bias_estimate, true_data, changing_bias_gt_estimate)
    ...
    calc_changing_int_bias_error += changing_bias_int_gp.print_error(alpha, sensor_bias, gaussian.underlyingData, changing_bias_int_estimate, true_data, changing_bias_int_gt_estimate)
    ...
    calc_both_error += calc_both_changing_bias_gp.print_error(alpha, sensor_bias, gaussian.underlyingData, calc_both_changing_bias_estimate, true_data, calc_both_changing_bias_gt_estimate)
    ...
    opt_theta_error += opt_theta.print_error(alpha, sensor_bias, gaussian.underlyingData, opt_theta_estimate, true_data, opt_theta_gt_estimate)
    ...
    opt_all_error += opt_all.print_error(alpha, sensor_bias, gaussian.underlyingData, opt_all_estimate, true_data, opt_all_gt_estimate)
